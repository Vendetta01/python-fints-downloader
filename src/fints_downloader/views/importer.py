...
class ImportFinTSGenericView(FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        fd_backend_url_connect = format_backend_url(
            fd_backend_base_url,
            'connect')
        ...
        if not connection_error:
            ...
            for key in ('fd_backend_url', 'fd_backend_payload',
                        'bank_login_id', 'tan_challenge',
                        'import_data_callback', 'account_id'):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            bank_login = BankLogin.objects.filter(
                pk=bank_login_id).first()
    ...
    def payload_from_post(self, backend_connection, backend_account, post_data):  # noqa: E501
        return GenericIn(
...
class TANView(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            fd_backend_payload = request.session.get('fd_backend_payload')
            fd_backend_url = request.session.get('fd_backend_url')
            import_data_callback = request.session.get(
            ...
            if not tan or\
                    not fd_backend_payload or\
                    not fd_backend_url or\
                    not form or\
                    not bank_login:
                raise Exception(
                    "TODO: something is not correctly set in session")
